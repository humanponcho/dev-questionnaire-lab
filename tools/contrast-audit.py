...
#
# This app has no terminal and no analyser rows, so --signal-warn and
# --signal-err carry less text here than in the sibling labs. The log
# classes in .pg-out are the only place they appear as type.
SURFACES = ['--black', '--deep', '--panel', '--glass', '--g0', '--signal-note']
WASHES   = [('--wash-ok', '--glass'), ('--wash-err', '--glass')]
...
SHAPE = {
    ...
    '--signal-warn':      ['--panel', '--g0'],             # .pg-dot.y
    '--signal-err-deep':  ['--g0'],                        # .pg-dot.r, on .pg-bar
    '--signal-note':      ['--panel', '--g0'],             # .progress-fill, active rail
}
