# Visual System — Dev Questionnaire Lab
...
Any hard-coded colour outside `:root` is a bug, with one documented exception — see [The live preview](#the-live-preview).
...
Quiz feedback and playground log levels **encode information in colour**. Removing the colour
would remove the information.

| Token | Meaning | Used by |
| --- | --- | --- |
| `--signal-ok` | Success, correct, passing | Quiz correct answer, `$` prompt, completion tick, `tip` callout, the green dot |
| `--signal-err` | Failure, wrong, blocking | Quiz wrong answer, error log lines, `warn` callout, the red dot |
| `--signal-warn` | Caution | Warning log lines, the amber dot |
...
amber accent used across the whole interface:

- Brand dot, progress bar fill, active sidebar item and its number
- Section eyebrows, quiz headings, prose bullets
- Inline code, callout code, quiz-explanation code
- The `▶ Run` button fill, Prev/Next hover border
- Home hero emphasis and card numbers

**Both of its values are the same colour.** Hue 37°, saturation 42%. Only the
lightness differs — 29% on paper, 60% on a dark screen.

```css
--signal-note: light-dark(#6a522b, #c4a46e);
```

#### Why this hue and not the Docker lab's blue

The migration standardised the *system*, not the brand. This app keeps the hue
of its pre-migration accent (`#f5a623`, hue 37°) and adopts the system's
...
The saturation is not negotiable. The old value was 91%, which reads as
...
`--signal-err` carries **text** — error log lines and the ✗ on a wrong answer. Text needs 4.5:1. On this
near-black dark ground that floor pushes the red up to lightness 59%, which
...
Do not use it for text. It reaches 4.06:1 on `--panel` but only 3.77:1 on
the error wash and 3.21:1 on `--g0` — short of 4.5:1 on both.

## The live preview

`.pg-preview` in the React playground is the one surface in the app that is
**not** tokenised, and deliberately so. It renders student-authored JSX. It has
to be a neutral white page in both schemes, or the student's own colours are
judged against a ground that moves when the operating system appearance changes.

Because the ground is fixed white, its text is fixed near-black. Do not swap
either for a token. Everything else in `.pg` around it still follows the scheme.
...
| --- | --- | --- |
| `--signal-ok` | `--panel`, `--glass`, `--wash-ok` over `--glass` | the wash |
| `--signal-err` | `--panel`, `--glass`, `--wash-err` over `--glass` | the wash |
...
before measuring. Here that is `--glass`, the ground under an answered quiz row:

| Wash | Light | Dark |
| --- | --- | --- |
| `--wash-ok` over `--glass` | `#d8ddd3` | `#1a2922` |
| `--wash-err` over `--glass` | `#e6d8d3` | `#2c1e1c` |

Measured against the current token values, each on its darkest surface:

| Token | Scheme | Value | Darkest surface | Ratio |
| --- | --- | --- | --- | --- |
| `--signal-ok` | light | `#55651a` | `#d8ddd3` | 4.66:1 |
| | dark | `#879e2b` | `#1a2922` | 5.03:1 |
| `--signal-err` | light | `#992918` | `#e6d8d3` | 5.63:1 |
| | dark | `#e2614d` | `#2c1e1c` | 4.62:1 |
| `--signal-warn` | light | `#9d4d1b` | `#e4e3dc` | 4.65:1 |
| | dark | `#eb7024` | `#1b1b18` | 5.65:1 |
| `--signal-err-deep` | dark | `#dd442c` | `#2e2e2a` | 3.21:1 (shape, floor 3:1) |
| `--signal-note` | light | `#6a522b` | `#cecdc6` | 4.61:1 |
| | dark | `#c4a46e` | `#2e2e2a` | 5.77:1 |

The `▶ Run` button is the one place a surface token becomes foreground: its
label is `--black` on a `--signal-note` fill. That pairing measures 6.38:1
on paper and 8.23:1 on phosphor.
...
| Token | Family | Job |
| --- | --- | --- |
| `--mono` | Share Tech Mono | Code, instrument labels |
...
6. Components — playground shell, buttons, editors, quiz
7. This app's own panels — the live preview
...
- [ ] No hard-coded colour outside `:root` (except `.pg-preview`).
